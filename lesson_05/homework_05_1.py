...
filtered_cars = {car: data for car, data in car_data.items() if data[1] >= search_criteria[0]
                 and data[2] >= search_criteria[1] and data[4] <= search_criteria[2]}
sorted_cars = sorted(filtered_cars.items(), key=lambda item: item[1][4])
first_five_cars = sorted_cars[:5]
print('''A selection of the first 5 cars that meet the criteria: 
"year of manufacture at least 2017, engine volume at least 1.6, price no more than 36,000"
Sorting by "Price"''')
print('\n'.join(f"{car}: {data}" for car, data in first_five_cars))
